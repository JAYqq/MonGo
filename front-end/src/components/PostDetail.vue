          ...
          <header class="g-mb-30">
            ...
            <ul class="list-inline d-sm-flex g-color-gray-dark-v4 mb-0">
              ...
              <li v-if="post.author" class="list-inline-item">
                <router-link v-bind:to="{ name: 'UserOverview', params: { id: post.author.id }}" class="u-link-v5 g-color-gray-dark-v4 g-color-primary--hover g-text-underline--none--hover"><span v-if="post.author.name">{{ post.author.name }}</span><span v-else>{{ post.author.username }}</span></router-link>
              </li>
...
export default {
  ...
  methods: {
    ...
    onSubmitAddComment:function(e){
      this.commentForm.errors=0
      console.log("body:"+this.commentForm.body)
      if (!this.commentForm.body){
        this.commentForm.errors++
        this.commentForm.bodyError="Body is required"
        $('#addCommentForm .md-editor').closest('.form-group').addClass('u-has-error-v1')
      }else{
        this.commentForm.bodyError=null
        $('#addCommentForm .md-editor').closest('.form-group').removeClass('u-has-error-v1')  
      }
    ...
    onClickReply(comment){
      ...
      console.log("authorname:"+this.commentForm);
    },
    onDeleteComment(comment){
      this.$swal({
        title:"Are you sure?",
        text:"删除评论会删除所有的子评论，你可以使用【屏蔽】功能",
        type:"warning",
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if(result.value){
          ...
          this.$axios.delete(path)
          .then((response)=>{
            this.$swal('Deleted','You are successfully delete this comment!')
            this.getPostComments(this.$route.params.id)
          })
          .catch((error)=>{
            console.log(error.response.data)
